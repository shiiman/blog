...
- 日付/時刻のタイムゾーン整形（計画1の変換でフロントマターの `date`/`modified` は UTC 表記=`...T..:..:..Z` 形式になっている。瞬時としては元データと同一だが、一覧・記事ページでの表示タイムゾーンと整形フォーマットを計画2で決定する。例: JST で日付のみ表示するなら表示側で +09:00 整形する）

...
/* プライズリスト自動スクロール用アニメーション */
@keyframes prizeScroll {
  from { transform: translate3d(0, 0, 0); }
  to { transform: translate3d(0, var(--scroll-distance), 0); }
}
...
<div class="poker-timer-app" id="pokerTimer" style="display: block;">
  ...
  <button class="exit-fullscreen-btn" id="btnExitFullscreen" title="フルスクリーン解除" aria-label="フルスクリーン解除">✕</button>
  <div class="timer-grid">
    ...
    <div class="left-panel">
      ...
      <!-- 画像領域（横画面フルスクリーン時のみ表示）
           TODO: Base64埋め込みからWordPressメディアライブラリのURL参照への移行を推奨 -->
    ...
    <div class="center-panel">
      ...
      <div class="timer-time" id="timerDisplay" role="timer" aria-live="polite" aria-label="タイマー残り時間">10:00</div>

      <div class="progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="100" id="progressBarContainer">
      ...
      <div class="controls">
        <button class="btn btn-primary" id="startBtn" aria-label="タイマー開始/停止">開始</button>
        <button class="btn btn-secondary" id="btnPrev" aria-label="前のレベルに戻る">戻る</button>
        <button class="btn btn-secondary" id="btnSkip" aria-label="次のレベルに進む">進む</button>
        <button class="btn btn-warning" id="btnSettings" aria-label="設定を開く">設定</button>
      </div>
    </div>
...
<!-- 設定モーダル -->
<div class="settings-modal" id="settingsModal" role="dialog" aria-modal="true" aria-labelledby="settingsTitle" style="position:fixed!important;top:0!important;left:0!important;width:100%!important;height:100%!important;z-index:2147483647!important;">
  <div class="settings-content">
    <h2 class="settings-title" id="settingsTitle">タイマー設定</h2>
...
(function() {
  ...
  function updateLevelBadge(blind) {
    ...
    if (blind.isBreak) {
      ...
      // XSS対策: textContent で安全に設定
      function setLevelBadgeContent(badge, levelId, totalId) {
        badge.textContent = '';
        var prefix = document.createTextNode('Level ');
        var levelSpan = document.createElement('span');
        levelSpan.id = levelId;
        levelSpan.textContent = levelNum;
        var sep = document.createTextNode(' / ');
        var totalSpan = document.createElement('span');
        totalSpan.id = totalId;
        totalSpan.textContent = totalLevelsCount;
        badge.appendChild(prefix);
        badge.appendChild(levelSpan);
        badge.appendChild(sep);
        badge.appendChild(totalSpan);
        badge.classList.remove('break');
      }
      if (lb) setLevelBadgeContent(lb, 'currentLevel', 'totalLevels');
      if (lbMobile) setLevelBadgeContent(lbMobile, 'currentLevelMobile', 'totalLevelsMobile');
    }
  }
  ...
  function updateDisplay() {
    ...
    var pbc = $('progressBarContainer');
    if (pbc) pbc.setAttribute('aria-valuenow', Math.round(progress));
    ...
    if (nb) {
      ...
      nb.textContent = '';
      if (nextBlindData) {
        nb.appendChild(document.createTextNode(nextBlindData.sb + ' / ' + nextBlindData.bb + ' '));
        var anteSpan = document.createElement('span');
        anteSpan.className = 'next-ante';
        anteSpan.textContent = '(Ante: ' + nextBlindData.ante + ')';
        nb.appendChild(anteSpan);
      } else {
        nb.textContent = '--';
      }
    }
  ...
  function updatePrizeDisplay() {
    ...
    if (pi) {
      // XSS対策: DOM APIで安全に設定
      pi.textContent = '';
      pi.appendChild(document.createTextNode('インマネ: '));
      var valueSpan = document.createElement('span');
      valueSpan.className = 'prize-inmoney-value';
      valueSpan.textContent = totalInMoney;
      pi.appendChild(valueSpan);
      pi.appendChild(document.createTextNode('名'));
    }
    ...
    if (listEl) {
      ...
      if (state.prizeDistribution.length > 0) {
        ...
        for (var k = 0; k < state.prizeDistribution.length; k++) {
          ...
          rankSpan.className = 'prize-rank';
          rankSpan.textContent = rankText;
          var amountSpan = document.createElement('span');
          amountSpan.className = 'prize-amount';
          amountSpan.textContent = p.amount.toLocaleString() + ' pt';
        ...
        for (var m = 0; m < prizes.length; m++) {
          ...
          rankSpan2.className = 'prize-rank';
          rankSpan2.textContent = (m + 1) + '位';
          var amountSpan2 = document.createElement('span');
          amountSpan2.className = 'prize-amount';
          amountSpan2.textContent = prizes[m].toLocaleString() + ' pt';
          item2.appendChild(rankSpan2);
          item2.appendChild(amountSpan2);
  ...
  // 無限ループスクロール（CSS Animationベース）
  function startPrizeAutoScroll() {
    // 旧 setInterval があれば停止
    ...
    // CSS Animation でスクロール（20px/秒の速度）
    var duration = loopHeight / 20;
    inner.style.setProperty('--scroll-distance', '-' + loopHeight + 'px');
    inner.style.animation = 'prizeScroll ' + duration + 's linear infinite';
  }
  ...
  // 共通ドラッグ&ドロップ ハンドラ生成
  function createDragHandlers(type, itemSelector, onDrop) {
    ...
    function drop(e) {
      ...
      if (state.draggedItem !== this) {
        onDrop(state.draggedItem, this);
      }
      return false;
    }
    return { dragStart: dragStart, dragEnd: dragEnd, dragOver: dragOver, dragLeave: dragLeave, drop: drop };
  }

  // 共通: コンテナ内のアイテムにドラッグイベントを設定
  function setupDragAndDrop(containerId, itemSelector, handlers) {
    var container = $(containerId);
    if (!container) return;
    var items = container.querySelectorAll(itemSelector);
    for (var i = 0; i < items.length; i++) {
      items[i].addEventListener('dragstart', handlers.dragStart);
      items[i].addEventListener('dragend', handlers.dragEnd);
      items[i].addEventListener('dragover', handlers.dragOver);
      items[i].addEventListener('dragleave', handlers.dragLeave);
      items[i].addEventListener('drop', handlers.drop);
    }
  }

  // プライズのドラッグ&ドロップ
  var prizeDragHandlers = createDragHandlers('prize', '.prize-edit-item', function(draggedItem, targetItem) {
    var container = $('prizeEditList');
  ...
  function setupPrizeDragAndDrop() {
    setupDragAndDrop('prizeEditList', '.prize-edit-item', prizeDragHandlers);
  }
  ...
  function addPrizeRow() {
    ...
    // イベント設定（共通ハンドラを使用）
    item.addEventListener('dragstart', prizeDragHandlers.dragStart);
    item.addEventListener('dragend', prizeDragHandlers.dragEnd);
    item.addEventListener('dragover', prizeDragHandlers.dragOver);
    item.addEventListener('dragleave', prizeDragHandlers.dragLeave);
    item.addEventListener('drop', prizeDragHandlers.drop);
  ...
  // ブラインドのドラッグ&ドロップ（共通ハンドラを使用）
  var blindDragHandlers = createDragHandlers('blind', '.blind-level-item', function(draggedItem, targetItem) {
    var container = $('blindLevels');
  ...
  function setupBlindDragAndDrop() {
    setupDragAndDrop('blindLevels', '.blind-level-item', blindDragHandlers);
  }
  ...
  function playSound() {
    ...
    try {
      ...
      chimeAudio.volume = 1;
      chimeAudio.play().catch(function() { /* 再生失敗は無視 */ });
    } catch (e) {}
  }
    ...
    if (canFullscreen) {
      ...
      if (!isFullscreen) {
        if (app.requestFullscreen) {
          app.requestFullscreen().catch(function() { /* フルスクリーン拒否時は無視 */ });
        } else if (app.webkitRequestFullscreen) {
          app.webkitRequestFullscreen();
        }
      } else {
        if (document.exitFullscreen) {
          document.exitFullscreen().catch(function() { /* 終了失敗時は無視 */ });
  ...
  function setMobileFullscreenHeight() {
    ...
    if (grid) {
      // パディングを考慮した幅を計算
      var paddingLeft = parseFloat(window.getComputedStyle(app).paddingLeft) || 8;
      var paddingRight = parseFloat(window.getComputedStyle(app).paddingRight) || 8;
  ...
  function handleKeyDown(e) {
    // 入力フィールド・編集可能要素・モーダル表示中はショートカット無効

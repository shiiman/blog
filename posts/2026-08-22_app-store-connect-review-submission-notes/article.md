...
私は #362（リリース要件のトラッキング Issue）に「アプリ名の確保」項目そのものを入れ忘れていて、後から追記しました。しかもその後、**5 日間チェックが倒れていない状態で放置**していました。実際にはコメントが「次にやること」として挙げた **8 分後に作成が完了していた**のです。
...
さらに私自身、`limit=8` の一覧を見て「このビルドは App Store Connect に存在しない」と判断しました。実際は `VALID` で存在していました。

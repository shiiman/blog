...
正直に言うと、アプリのコードを書き終えてからが本番でした。実装は「動くものを作る」だけで済みますが、ストアに出すには契約・税務・署名・課金商品の登録・年齢レーティング・プライバシー申告・審査ノートという、コードとまったく関係ない山を越える必要があります。しかもこの領域には、次のような特徴があります。

- 公式ドキュメントに載っていない中間ステップがある
- 成功レスポンスが返っているのに実際は何も起きていない
- 画面の表示が「完了」でも実際は未完了

といった罠が、とにかく多かったです。ここで何度も「終わったつもり」になって、後から巻き戻しました。
...
属性を空にした POST を投げると、有効値が全部返ってきます。

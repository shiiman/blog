...
いまは両ストアとも承認済みで、公開ボタンを押すだけの状態です。**公開操作については、実際に押したあとにこの記事へ追記**します。
...
私はこれを 2 回読み違えました。1 回目は「承認と同時に配信開始」と書き、指摘されて覆したのに、2 回目に同じ誤りを繰り返しています。
...
承認済みだと分かった直後に、私はそう言いました。**これも誤りでした。** 実測すると本当に約 7 日かかっていました。
...
**両ストアとも承認が出て、あとは公開ボタンを押すだけの状態**になりました。まだ押していません。

| ストア | 承認後の状態 | 押す場所 |
|---|---|---|
| App Store | `PENDING_DEVELOPER_RELEASE` | App Store Connect のバージョンページ |
| Google Play | 「変更の公開準備完了」+ 青い「N 件の変更を公開」 | Play Console の `公開の概要` |

**どちらも「承認 = 公開」ではありません。** Apple は `releaseType = MANUAL` にしてあるので止まります。Play は初回リリースだと、チェックリストに「リリースを審査のために Google に送信する」と「Google Play にアプリを公開する」が別ステップで並びます。

ここで 1 つ勘違いしていました。「管理対象の公開: オフ」のツールチップに「承認されるとすぐに自動的に公開されます」と書いてあるのを読んで、Play は自動公開だと思い込んでいたのです。**この文は「アップデートは」で始まっていて、初回リリースには当てはまりません。** そう書いてあるのに読み飛ばしていました。

結果として、**両ストアの公開タイミングは開発者側が握れます**。2 つ押すだけで同時公開になります。
...
両ストアに同時に出して得たものを 7 つ。

1. **Apple のほうが速く、Play のほうが遅かった。** 提出から承認まで Apple が 2 日 14 時間（1 回却下を挟んで）、Play が 6 日 18 時間（一発通過）。先入観とまるごと逆でした
2. **Play は審査結果をメールで送ってこない。** 審査状態をメールの有無で判断してはいけません
3. **Play の審査ステータスは API から読めない。** `公開の概要` の「送信アクティビティ」だけが「いつ終わったか」を持っています
...
公開操作が済んだら追記します。同じところで消耗する人が 1 人でも減れば幸いです。

...
draft: false
id: 2251
excerpt: 個人開発アプリをApp StoreとGoogle Playに同じ時刻に申請して公開するまでの記録です。AppleはGuideline 2.1（Information Needed）で1回落ちてから2日14時間で承認、Playは指摘ゼロの一発承認でしたが6日18時間かかりました。却下理由の全文、両ストアの審査時間の比較、Playの状態を測る唯一の方法、公開ボタンを押したあとに何が起きたかまでまとめています。
...
完了の 80 分後に受信箱を検索して 0 件でした（個人アカウントと転送先の両方）。同じ期間に Apple からは、提出の受付 1 通・却下 2 通・承認 2 通の計 5 通が届いています。
...
| ページが見えるまで | 遅くとも 27 分 36 秒 | 遅くとも 1 分 59 秒 |
...
+ 1 分 59 秒        IARC から通知が届く。このとき Play のページは既に 200
                    App Store のページはこの時点ではまだ出ていない
+ 27 分 36 秒       HTTP を測ると App Store のページも 200 になっていた
```

**待たされたのは App Store だけです。** Play は 2 分の時点でもう見えていたのに、App Store はそのとき出ていませんでした。記録上は 1 分 5 秒でリリース済みになっているにもかかわらずです。

ただし、**どちらも「遅くとも」しか言えません。** 見えるようになった瞬間を測っていないからです。記録に残っているのは「1 分 59 秒の時点で Play は 200」「27 分 36 秒の時点で App Store も 200」の 2 点だけで、その間のどこで切り替わったかは分かりません。体感では App Store は 10 分ほどでしたが、それは測った値ではないので数字として置きません。

この記事で何度も書いている「観測していないものを断定しない」は、自分の体感にも同じように効きます。**正確に知りたいなら、押した直後から両方のストアページの HTTP を定期的に測るしかありませんでした。**

この 2 つは別物です。API が `READY_FOR_SALE` を返していても、ストアページはまだ出てこないことがあります。**「リリースした」と「ユーザーが見つけられる」は分けて測る**必要がありました。24 時間という上限に対して、実際はどちらも 30 分かかっていません。
...
Play 側にこれに当たるものはありません。IARC の通知も公開の時刻そのものではなく、**遅れて届く裏づけ**です。
...
8. **公開ボタンを押してからは速い。** 「最大 24 時間」は上限でした。ページが 200 になっていたのを確認できたのは Play が 2 分、App Store が 28 分の時点です。しかも Apple は記録上 1 分でリリース済みなので、**リリース済みとページが見えるのは別**です

...
draft: false
id: 2251
excerpt: 個人開発アプリをApp StoreとGoogle Playに同じ時刻に申請して公開するまでの記録です。AppleはGuideline 2.1（Information Needed）で1回落ちてから2日14時間で承認、Playは指摘ゼロの一発承認でしたが6日18時間かかりました。却下理由の全文、両ストアの審査時間の比較、Playの状態を測る唯一の方法、そして公開ボタンを押したあとに何が起きたか（ページが見えたのはPlayが2分・App Storeが28分の時点で確認・Appleは記録上1分なのでリリース済みとページが見えるのは別・公開状態がAPIに出るのはAppleだけ・両ストアともメールは来ない・curlの301の罠）までまとめています。

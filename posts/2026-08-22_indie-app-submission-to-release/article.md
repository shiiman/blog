...
draft: false
id: 2251
excerpt: 個人開発アプリをApp StoreとGoogle Playに同じ時刻に申請して公開するまでの記録です。AppleはGuideline 2.1（Information Needed）で1回落ちてから2日14時間で承認、Playは指摘ゼロの一発承認でしたが6日18時間かかりました。却下理由の全文、両ストアの審査時間の比較、Playの状態を測る唯一の方法、そして公開ボタンを押したあとに何が起きたか（Playは直後に見えてApp Storeだけ10分・Appleは記録上1分なのでリリース済みとページが見えるのは別・公開状態がAPIに出るのはAppleだけ・両ストアともメールは来ない・curlの301の罠）までまとめています。
...
| ページが見えるまで | 約 10 分 | すぐ（押した直後に 200） |
...
```
押した時点         両ストアの公開ボタンを押す
すぐ               Play のストアページが 200 を返す
+ 1 分 5 秒        App Store 側にリリースが記録される（ページはまだ見えない）
+ 1 分 59 秒       Play のレーティングが live になった通知が IARC から届く
+ 約 10 分         App Store のページがようやく見えるようになる
```

**Play は押した直後にページが見えました。** 待ち時間はありません。

**遅れたのは App Store だけです。** しかも記録上は 1 分 5 秒でリリース済みになっているのに、ページが出てくるまで 10 分ほどかかりました。
...
Play 側にこれに当たるものはありません。ただし**押した直後からストアページが 200 を返す**ので、HTTP を測れば十分でした。IARC の通知はページが見えるようになった後に届いているので、公開の時刻そのものではなく、**遅れて届く裏づけ**として扱うのが正しいです。
...
8. **公開ボタンを押してからは速い。** 「最大 24 時間」は上限でした。Play は押した直後にページが見え、App Store だけが 10 分ほどかかっています。しかも Apple は記録上 1 分でリリース済みなので、**リリース済みとページが見えるのは別**です

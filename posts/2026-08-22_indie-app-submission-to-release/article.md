...
draft: false
id: 2251
excerpt: 個人開発アプリをApp StoreとGoogle Playに同じ時刻に申請して公開するまでの記録です。AppleはGuideline 2.1（Information Needed）で1回落ちてから2日14時間で承認、Playは指摘ゼロの一発承認でしたが6日18時間かかりました。却下理由の全文、両ストアの審査時間の比較、Playの状態を測る唯一の方法、そして公開ボタンを押したあとに何が起きたか（Appleだけ API に出る・両ストアともメールは来ない・curlの301の罠）までまとめています。
...
| ストアページ | 同じ日のうちに見えた | 同じ日のうちに見えた |
| 公開を知らせるメール | **0 通** | **0 通** |

Apple の承認メールには「リリースしてからストアに出るまで最大 24 時間」とありましたが、実際は同じ日のうちに両方とも見えるようになりました。

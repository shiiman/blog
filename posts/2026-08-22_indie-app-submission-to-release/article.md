...
draft: false
id: 2251
excerpt: 個人開発アプリをApp StoreとGoogle Playに同じ時刻に申請して公開するまでの記録です。AppleはGuideline 2.1（Information Needed）で1回落ちてから2日14時間で承認、Playは指摘ゼロの一発承認でしたが6日18時間かかりました。却下理由の全文、両ストアの審査時間の比較、Playの状態を測る唯一の方法、そして公開ボタンを押したあとに何が起きたか（ストア側の記録は1〜2分・App Storeのページが見えるまで10分・Appleだけ API に出る・両ストアともメールは来ない・curlの301の罠）までまとめています。
...
| ストア側にリリースが記録された | 押してから 1 分 5 秒 | 押してから 1 分 59 秒（IARC 通知の時刻） |
| ページが実際に見えるまで | 約 10 分 | すぐ |
| 公開を知らせるメール | 0 通 | 0 通 |

### 「リリース済み」とページが見えるのは別だった

承認メールには「リリースしてからストアに出るまで最大 24 時間」とありました。実際に測ると、こうでした。

```
押した時点          両ストアの公開ボタンを押す
+ 1 分 5 秒         App Store 側のリリース記録が立つ
+ 1 分 59 秒        Play のレーティングが live になった通知が届く
+ 約 10 分          App Store のページが実際に見えるようになる
```

Play はページがすぐ反映されました。**App Store は記録上 1 分で公開済みなのに、ページが見えるまで 10 分ほどかかっています。**

この 2 つは別物です。API が `READY_FOR_SALE` を返していても、ストアページはまだ出てこないことがあります。**「リリースした」と「ユーザーが見つけられる」は分けて測る**必要がありました。24 時間は上限であって、実際はどちらも桁が 2 つ小さかったです。

Apple 側の正確な時刻は Lookup API から取れます。認証は要りません。
...
Play 側には同じものがありません。公開の時刻を裏づける手がかりは、**IARC から届いた通知のタイムスタンプだけ**でした。
...
8. **公開ボタンを押してからは速い。** 「最大 24 時間」は上限で、実際はストア側の記録が 1〜2 分、App Store のページが見えるまでが 10 分ほどでした。ただし、**リリース済みとページが見えるのは別**です

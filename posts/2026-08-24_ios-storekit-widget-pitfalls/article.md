...
date: '2026-08-24T20:00:00+09:00'
categories:
  - app
tags:
  - storekit
...
レビューの観点として「ウィジェットや Siri で非公開のバケットを出すのは PII の漏洩ではないか」という論点が挙がりました。これは**却下しました**。

このアプリは単一ユーザーの個人データアプリです。**「ユーザー自身のデータを、ユーザー自身の端末やユーザーが明示的に連携した外部サービスに出す」ことは第三者への漏洩ではなく、機能の目的そのもの**です。

この線引きを先にドキュメントに書いておいたのが効きました。同じ論点が出てくるたびに、そこを引用して判断できます。ロック画面への露出や Siri の読み上げというシナリオも検討したうえで、**オーナー基準で許容**と決めました。ウィジェットはタイムラインでロック画面とホーム画面を区別できないので、「出す / 出さない」の二択しかないという事情もありました。
